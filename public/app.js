...
function pointRole(point) {
  if (point.region === "buenos-aires") return "referencia urbana";
...
function renderCard(item) {
  ...
  card.className = `card ${isReference ? "card-reference" : ""}`;
  card.innerHTML = `
    <div class="card-head">
...
function renderLongRange() {
  ...
  const visible = asArray(state.payload.boards)
    ...
    .sort((a, b) => {
      if (a.point.region === "buenos-aires") return 1;
      if (b.point.region === "buenos-aires") return -1;
      return (b.summary.gradeScore ?? 0) - (a.summary.gradeScore ?? 0);
    });
  ...
  els.longrangePlan.innerHTML = `
    <div class="matrix-intro">
      ...
      <div class="legend muted">Cada celda resume el día con score, icono dominante, temperatura, lluvia y viento.</div>
    </div>
  `;
                    ...
                    <div class="calendar-row ${isReference ? "calendar-row-reference" : ""}" style="grid-template-columns:${columnTemplate}">
                      ...
                      ${headerDates
                        .map((date) => {
                          ...
                          return `
                            <button class="day-cell ${scoreClass(day.gradeScore)} ${date === state.selectedDate ? "active" : ""}" type="button" data-day-jump="${date}" title="${item.point.name} · ${formatDay(day.date)}">
    ...
    <div class="detail-card">
      <div class="panel-head">
        <div>
          ...
          <p class="muted">${pointRole(item.point)} · ${item.point.aliases.join(" · ")}</p>
        </div>
        <div class="pill-list">
          <span class="pill">${item.primary.sourceLabel}</span>
      ...
      <div class="detail-grid">
        ...
        <div class="detail-side">
          <div class="detail-block">
            ...
            <p class="muted">Condición: ${conditionLabel(item.summary.condition)} · score ${item.summary.gradeScore}/100</p>
            <p class="muted">Riesgo dominante: ${dominantRisk(item.summary).label}</p>
